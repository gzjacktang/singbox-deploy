...
- ✅ **自动生成** - 自动生成密钥和配置文件，Reality SNI 可优选或手动输入
- ✅ **Reality SNI 优选** - 参考 3x-ui 的 TLS 1.3、h2、X25519、证书校验规则，使用单次 TLS 握手并发检测候选，对每个候选检测 3 次并按成功率和中位延迟排序
...
- ✅ **删除节点** - 在 `sb` 中安全删除指定协议的整个节点及其用户，配置校验和服务重启成功后才生效
...
不再内置或默认使用 `addons.mozilla.org`。严格检测要求目标同时满足 TLS 1.3、HTTP/2 `h2`、X25519 和可信且匹配的证书。扫描器不会再用 OpenSSL 与 curl 串行建立两次连接，而是通过一次 TLS 握手同时完成校验和计时；不同候选并发检测。每个目标检测 3 次，至少成功 2 次才会进入可选列表，延迟使用成功检测的中位数。扫描结束后始终可以选择手动输入其他 SNI；如果所有候选失败，会显示具体原因并转入手动输入。
